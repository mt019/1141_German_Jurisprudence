...
- 主句：*Hier soll gezeigt werden …*（無人稱，Vorgangspassiv，【虛主詞es被省略】）。  
- 不定式：*gezeigt werden*。  
- 賓語：*dass-Satz*。  
- 內層子句：*die Konstitutionalisierung … läuft hinaus*。  
  - 主語：*die Konstitutionalisierung der Rechtsordnung*。  
    - die in den vergangenen Jahrzehnten immer weiter vorangetriebene Konstitutionalisierung der Rechtsordnung
    - 【核心主詞】die Konstitutionalisierung
    - 【過去分詞做狀語】in den vergangenen Jahrzehnten immer weiter vorangetriebene
    - 改寫成形容詞子句： die Konstitutionalisierung, die in den vergangenen Jahrzehnten immer weiter vorangetriebenen wurde
  - 謂語：*läuft hinaus*（hinauslaufen auf + Akk. → 導向【導致、走向...（結果）】）。  
...
- 同位語：*dass-Satz*（它不能被收編）。  
  - dass sie sich rechtsdogmatisch nicht einfangen lässt 
    - 【改寫】dass sie rechtsdogmatisch nicht eingefangen werden kann
- 斷句後補充：*schlichte Konsequenz dessen …*（結果說明）。  
    - > 補充：*dessen* 是關係代名詞/指示代詞的屬格單數形式（陽/中性），在此為前指用法，所指的是後方 *dass*-子句整體，即「這裡有一個更高規範層次主張拘束力」這件事；整句可讀作「——這只是因為（＝這是……的結果）這裡有更高的規範層次在要求拘束。」
- 內層：*dass hier eine höhere Normschicht … beansprucht*。  
  - 主語：*eine höhere Normschicht*。  
  - 謂語：*beansprucht*。  
  - 補語：*Verbindlichkeit*。  
  - 比較：*als die des positiven Rechts*（比較層級）。
    - > 註：比較結構 *eine höhere Normschicht als die des positiven Rechts* 中的 *die* 省略了先行詞 *Normschicht*，指稱「（那個）規範層次」。換言之，後段可還原為 *als die Normschicht des positiven Rechts*，用來說明：此處的規範層次高於實證法自身的那一層。
    - > 進一步：片語 *des positiven Rechts* 由定冠詞屬格單數 *des*（對應中性名詞 *das Recht*）與形容詞 *positiv* 的屬格詞尾 *-en* 所組成，表示「實證法的」。
...
| 動詞 | 形式 | 意義 |
|---|---|---|
| einfangen | einfangen lässt | 被動式，不可收編、【原意：捕捉】 |
...
- 句子後半：*ist doch genau das …*。  
  - 系詞結構：*ist* 為核心，語氣詞 `doch` 用於強調「果然如此」。  
  - 指示結構：`genau das`（「恰恰就是這個」）中 `das` 回指前述 Tendenz。  
  - 表語片語：`die intendierte Konsequenz` 後接介詞片語 `einer ... Naturrechtslehre` 作定語，說明這結果「屬於自然法學說」。  
    - 改寫形容詞子句：*die Naturrechtslehre, die eine solche intendierte Konsequenz mit universellem Richtigkeitsanspruch entfaltet*（此處動詞 *entfalten* 表「展現、引出」，亦可換成 *bewirken*、*hervorrufen* 等）。
  - 定語內部：分詞 `auftretenden` 搭配介詞片語 `mit universellem Richtigkeitsanspruch`，描述「帶著普遍正當性訴求登場的自然法學說」。
...
| die Konsequenz | **die Konsequenz** | 陰 | 單 | 主格 | 結果 |
| der Anspruch (Richtigkeitsanspruch) | **der Anspruch** | 陽 | 單 | 主格 | 要求【請求權】 |
...
- **結構骨架**：`Der pastorale Ton ..., der ... gepflegt wird – das ist der hohe Ton des Vernunftrechts, wiedererstanden ...`，由主語群組 + 關係子句 + 判斷句三段組合。  
- **主語群組**：`Der pastorale Ton wohltemperierter Vernünftigkeit`
  - 核心名詞 *Ton*，由形容詞 *pastoral* 修飾。
  - 屬格片語 *wohltemperierter Vernünftigkeit* 進一步界定語調特徵。  
- **關係子句**：`der in den jüngeren Judikaten des BVerfG ostentativ gepflegt wird`
  - 關係代名詞 *der* 指向 *Ton*。
  - 被動式 *gepflegt wird* 表示語調被刻意維持；狀語 *ostentativ* 突出刻意性。
  - 介詞片語 *in den jüngeren Judikaten des BVerfG* 說明語調所在場域。  
- **主句判斷**：破折號後 `das ist ...`
  - 指示代名詞 *das* 回指前段語調描述。
  - 系詞 *ist* 建立等同關係。  
  - das ist der hohe Ton des Vernunftrechts, wiedererstanden in Gestalt der modernen Grundrechtsjurisprudenz
  - 【改寫】das ist der hohe Ton des Vernunftrechts, das in Gestalt der modernen Grundrechtsjurisprudenz wiederersteht
  - 將分詞片語改為關係子句；關係代名詞 *das* 指向 *Ton des Vernunftrechts*，動詞 *wiederersteht* 與原分詞 *wiedererstanden* 對應，仍表「重新出現」。
- **表語與補語**：`der hohe Ton des Vernunftrechts, wiedererstanden ...`
  - 表語 `der hohe Ton des Vernunftrechts`，內含屬格 *des Vernunftrechts* 說明來源。
  - 分詞片語 `wiedererstanden in Gestalt der modernen Grundrechtsjurisprudenz` 作狀語，描述「理性法的高音」以現代基本權法理學的形態復活。  
- **句法功能**：整句揭示 BVerfG 的語調等同理性法傳統的回歸，破折號營造「揭示／指出」的語氣。  
